  ...
  describe "change!" do
    ...
    context 'new timestamp is in the past' do
      ...
      it 'changes the timestamp of the topic and opening post' do
        ...
        topic.reload
        expect(topic.created_at).to eq(new_timestamp)
        expect(topic.updated_at).to eq(new_timestamp)
        expect(topic.bumped_at).to eq(new_timestamp)

        p1.reload
        expect(p1.created_at).to eq(new_timestamp)
        expect(p1.updated_at).to eq(new_timestamp)

        p2.reload
        expect(p2.created_at).to eq(new_timestamp + 1.day)
        expect(p2.updated_at).to eq(new_timestamp + 1.day)

        expect(topic.last_posted_at).to eq(p2.reload.created_at)
      end
      ...
      describe 'when posts have timestamps in the future' do
        it 'should set the new timestamp as the default timestamp' do
          ...
          expect(p3.created_at).to eq(new_timestamp)
          expect(p3.updated_at).to eq(new_timestamp)
        end
      end
    end
